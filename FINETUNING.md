...
| Learning rate | **pin to the donor's annealed LR** (`finetune.py` reads it from the checkpoint; typically the 1e-5 floor) | On resume, `AMP_PPO.__init__` resets the LR from the cfg (1e-3) and `update()` clobbers every param group on the first minibatch (`rsl_rl/algorithms/amp_ppo.py:153,360`). The adaptive-KL controller re-floors it within ~1 iteration, but pinning removes ~20 Adam steps at 100× the trained LR against a maximally-stale critic. Free insurance. The controller re-raises it if updates are tame. |
