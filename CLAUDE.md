...
# Fine-tune a trained checkpoint after an env/physics change (see FINETUNING.md for
# the knobs, monitoring bands, and failure modes — esp. the entropy-std ratchet)
uv run python scripts/finetune.py Mjlab-Skater-Flat-Agibot-X2 <ckpt.pt> --iters 1500
uv run python scripts/finetune_watchdog.py --total-iters 1500   # alongside, in a 2nd shell
uv run python scripts/eval_gate.py Mjlab-Skater-Flat-Agibot-X2 <ckpt.pt> --gate
